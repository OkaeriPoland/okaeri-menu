 ...
 *
 * <p>Strategies receive ALL filters and BaseMenuContext, and are responsible for:
 * <ul>
 *   <li>Checking which filters are active (via {@link ItemFilter#isActive(MenuContext)})</li>
 ...
 *     @Override
 *     public <T> Predicate<T> combine(BaseMenuContext context, Collection<ItemFilter<T>> filters) {
 *         return item -> {
...
public interface FilterStrategy {
     ...
     *
     * <p>Filters are checked via {@link ItemFilter#isActive(MenuContext)} before testing.
     */
    FilterStrategy AND = new FilterStrategy() {
        @Override
        public <T> @NonNull Predicate<T> combine(@NonNull MenuContext context, @NonNull Collection<ItemFilter<T>> filters) {
            return item -> {
                for (ItemFilter<T> filter : filters) {
                    // Strategy handles isActive() check with context
                    if (filter.isActive(context) && !filter.test(item)) {
     ...
     *
     * <p>Implementations MUST check {@link ItemFilter#isActive(MenuContext)} to determine
     ...
     *
     * @param context The menu context for accessing per-player state
     ...
     */
    <T> @NonNull Predicate<T> combine(@NonNull MenuContext context, @NonNull Collection<ItemFilter<T>> filters);
